...
import { EyeIcon, EyeSlashIcon, EnvelopeIcon, LockClosedIcon, UserGroupIcon } from '@heroicons/react/24/outline';
...
const StaffLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await login(formData, 'stafflogin');
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-luxury-600">
            Staff access to hotel operations system
          </p>
        </div>
        
        <div className="card-luxury p-8 border-l-4 border-primary-500">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
            ...
            <div className="border-t border-luxury-200 pt-6">
              ...
              <div className="flex flex-col space-y-2">
                <Link
                  to="/login"
                  className="text-center text-sm text-luxury-700 hover:text-primary-600 transition-colors duration-200"
                >
                  Guest Login
                </Link>
                <Link
